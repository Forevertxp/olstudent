...
function showMsg(msg) {
    ...
    var maxDisplayMsgCount = 7; //显示的评论条数
...
function independentModeAutoLogin(userId) {
    $.ajax({
		url:"/ola/tls/getTLSSignature",
		data:{"userId":userId},
		type:"post",
		dataType:"json",
		success:function(result){
			if(result.apicode==10000){
				loginInfo.identifier = result.loginInfo.account;
			    loginInfo.userSig = result.loginInfo.sig;
			    sdkLogin();
			}
		}
	});
